...
import math
import pandas as pd
from helping_functions import calculate_part_dimensions, get_number_voxel, fill_2D_voxel_area_v2, fill_2D_voxel_intensity, get_2D_data_from_h5, dock_df_to_zero, get_true_min_maxX, get_true_min_maxY, get_2D_data_from_h5_filtered
import time
...
def create_voxel_h5(path_buildjob_h5, path_voxel_h5, part_name, voxel_size, num_voxel_layers, mode, max_slice_number_part, mode_df): #max_slice_number_part can be removed as soon as attribute version is working
            ...
            for n_vox_y_init in range(num_voxels_y): #iterating over number of voxels in y-direction
                ...
                for n_vox_x_init in range(num_voxels_x):#iterating over number of voxels in x-direction
                    ...
                    if mode == 'Area+Intensity':
                        array_area = fill_2D_voxel_area_v2(voxel_size, n_vox_x_init, n_vox_y_init, df,'Zeros')
                        array_intensity = fill_2D_voxel_intensity_v2(voxel_size, n_vox_x_init, n_vox_y_init, df,'Zeros')
                        ...
                        array_area = fill_2D_voxel_area_v2(voxel_size, n_vox_x_init, n_vox_y_init, df)
                    ...
                    print('filling slice number {} in voxel_{}_{}_{}'.format(num_slice, n_vox_x_init,n_vox_y_init,num_z))
        # y-Achse entspricht horizontal, x-Achse entspricht vertikal
        print("slice  took %s seconds ---" % (time.time() - start_time))
    return()



if __name__ == '__main__':
    create_voxel_h5('/home/jan/Documents/CodeTDMStoHDF/Ausgangsdaten/examplerRun.h5', '/home/jan/Documents/Voxel_Erstellung/voxels_test_big.hdf5', '0_00003_Canti3_cls', 200, 10, 'Area_only', 142,'mean')
